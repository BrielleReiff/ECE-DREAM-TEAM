	...
	while( run )
	{
		...
		for( vector<TLight*>::iterator iter = lights.begin(); iter != lights.end(); iter++ )
		{
			...
			float stopline_min = 100;
				...
				{
				double time_to_green = 99;
				//calculate time to next green
			...
			if(stopline_min <= 0.6 && (traffic->status[4] == 1 || traffic->status[4] == 4) && continue_flag==true){
			...
			if(continue_flag==false){		
				...
				if (dist_to_center > 4.0){
					continue_flag = true; //Monitor the traffic light again
				}
		...
		if( ob_valid_foundall )
		{
			for( int i = 0; i < MAX_ROBOTS; i++ )
			{
				...
				if( ob_dist != 0 )
				{
					...
					// If the object is in the direction of Car0 and not going through the intersection
					if( ob_dist != 0 && dot_prod2 > 0.0 && sign_flag == false)
					{
		...
		printf( "Speed = %f, steer = %f stop is %s powerokay is %s, continue_flag: %i\n", controller->getSpeed(), -1.0 * controller->getSteeringRate(), (stop ? "true" : "false" ), ( powerokay ? "true" : "false" ), continue_flag );
